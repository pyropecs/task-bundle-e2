...
public class BookControllerTest extends TestConfig {
  ...
  @Mock
    ...
    public void postRequestCreateUserTest() throws Exception{
        mockMvc.perform(MockMvcRequestBuilders.post("/books/add").param("name", "60").param("author", "testauthor").param("price", "20")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/books")).andExpect(flash().attribute("message","Book Created Successfully" )).andExpect(flash().attribute("path", "books"));
